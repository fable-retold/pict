		...
		suite(
			...
			suite(
				...
				function()
				{
					...
					test(
						'_buildQueryReadBody URL-decodes the filter so POST /Query receives the value the GET route would have decoded',
						function()
						{
							const tmpProvider = new libPict(_MockSettings).EntityProvider;
							// A LIKE filter as it is built for the legacy GET reads: the
							// wildcards are %25 (encoded '%') and a value-borne space arrives
							// %20-encoded from outside. The GET route's :Filter path segment
							// gets URL-decoded by the server; POST /Query does not, so the
							// provider must decode it before putting it in the body.
							const tmpEncodedFilter = 'FBV~Name~LK~%25Acme%20Corp%25';
							const tmpBody = tmpProvider._buildQueryReadBody(tmpEncodedFilter, 0, 100, null);
							Expect(tmpBody.Filter).to.equal('FBV~Name~LK~%Acme Corp%');
						}
					);

					test(
						'_buildQueryReadBody falls back to the raw filter on a malformed percent-sequence',
						function()
						{
							const tmpProvider = new libPict(_MockSettings).EntityProvider;
							// A lone '%' is not a valid escape; decodeURIComponent throws.
							// The POST path must degrade to the raw string (never worse than GET).
							const tmpBadFilter = 'FBV~Name~LK~%25Bob %Builder%25';
							const tmpBody = tmpProvider._buildQueryReadBody(tmpBadFilter, null, null, null);
							Expect(tmpBody.Filter).to.equal(tmpBadFilter);
						}
					);

					test(
						'supported server: POST /Query body carries the decoded filter while GET would carry the encoded one',
						function(fDone)
						{
							const tmpProvider = new libPict(_MockSettings).EntityProvider;
							const tmpStubs = installRestStubs(tmpProvider, _SchemaSupportedFlag);
							...
							tmpProvider.getEntitySetRecordCount('Book', tmpEncodedFilter,
								(pError, pCount) =>
								{
									try
									{
										...
										const tmpCountCall = tmpStubs.postStub.getCalls().find((pCall) => { return pCall.args[0].body && pCall.args[0].body.Count; });
										Expect(tmpCountCall).to.exist;
										// The body filter is decoded — not the encoded GET-shaped string.
										Expect(tmpCountCall.args[0].body.Filter).to.equal('FBV~Name~LK~%Acme Corp%');
									}
									catch (pAssertError)
									{
										return fDone(pAssertError);
					...
					test(
						'unsupported server: GET read passes the encoded filter through verbatim in the URL',
						function(fDone)
						{
							const tmpProvider = new libPict(_MockSettings).EntityProvider;
							const tmpStubs = installRestStubs(tmpProvider, _SchemaLegacyNoMetadata);
							...
							tmpProvider.getEntitySetRecordCount('Book', tmpEncodedFilter,
								(pError, pCount) =>
								{
									try
									{
										Expect(pError).to.not.exist;
										Expect(tmpStubs.postStub.callCount).to.equal(0);
										// The GET URL still embeds the encoded filter; the server URL-decodes it.
										const tmpURLs = tmpStubs.getStub.getCalls().map((pCall) => { return (typeof pCall.args[0] === 'string') ? pCall.args[0] : pCall.args[0].url; });
										Expect(tmpURLs.some((pU) => { return pU.indexOf(`/Books/Count/FilteredTo/${tmpEncodedFilter}`) !== -1; })).to.equal(true);
									}
									catch (pAssertError)
									{
										return fDone(pAssertError);
